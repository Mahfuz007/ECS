...
class Exams extends Controller
{
    ...
    public function show($id)
    {
        ...
        $examConfirm = $this->userModel->getExamConfirm($_SESSION['id'], $id);
        if (empty($examConfirm)) {
            $exam->confirm = -1;
        } else {
            $exam->confirm = $examConfirm->confirm;
        }
        ...
        if (date("Y-m-d H:i:s") >= $exam->begin_time && date("Y-m-d H:i:s") <= $exam->end) {
            if ($exam->confirm != 1) {
                setFlash('unauthorize', "You are not allowed to participate in the exam");
                redirect('');
            }
        }
    ...
    {
        $this->userModel->register($_SESSION['id'], $id);
        redirect('exams/show/' . $id);
    }

    public function review($eid)
    {
        //authenticate exam author
        $author = $this->userModel->getInfo($eid)->author;
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            if ($_POST['action'] == 'accept') {
                $this->userModel->acceptReg($_POST['sid'], $eid);
            } else {
                $this->userModel->rejectReg($_POST['sid'], $eid);
            }
        }

        $data = $this->userModel->review($eid);
        $this->view('exams/review', $data);
    }
}
